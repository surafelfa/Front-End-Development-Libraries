class Clock extends React.Component {
    ...
    this.setState({
      intervalID: setInterval(() => {
        if (this.state.timeCounter === 0) {
          //session should start as break ends
          this.setState((state) => ({
  ...
  startSession() {
    this.setState({
      intervalID: setInterval(() => {
        if (this.state.timeCounter === 0) {
          //break should start as session ends
          this.setState((state) => ({
    ...
    return (
      <div className="main-wrapper">
        ...
        <div className="break-session-wrapper">
          <span className="break-wrapper">
            ...
            <i
              ...
              onClick={this.breakDecrementClicked}
            ></i>
            <h3 id="break-length"> {this.state.breakLenght} </h3>
            <i
              id="break-increment"
              className="fas fa-arrow-up"
              onClick={this.breakIncrementClicked}
            ></i>
          </span>
          <div id="spacer">&emsp;&emsp;</div>
          <span className="session-wrapper">
            <h2 id="session-label">Session Length</h2>
            <i
              id="session-decrement"
              className="fas fa-arrow-down"
              onClick={this.sessionDecrementClicked}
            ></i>
            <h3 id="session-length"> {this.state.sessionLength} </h3>
            <i
              id="session-increment"
              className="fas fa-arrow-up"
              onClick={this.sessionIncrementClicked}
            ></i>
          </span>
        </div>
        <div className="timer">
        ...
        <div className="clickable">
          <button id="start_stop" onClick={this.startTimeCounter}>
            <span id="play-pause">
